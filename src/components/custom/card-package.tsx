...
export const CardPackage = ({
  ...
  return (
    <Button
      ...
      } ${highlighted ? "scale-110 shadow-lg border-4 border-pink-500" : ""}`}
    >
      <div className="flex flex-col p-4 gap-14">
        <div>
          <p className="pt-4 text-3xl font-medium">{time}</p>
        </div>
        <div className="w-56 py-4 px-16 bg-gradient-to-t from-[#3A1852] to-[#C70655] rounded-2xl">
          <p className="text-3xl text-white font-medium">P {amount}.00</p>
        </div>
      </div>
    </Button>
  );
